...
from history_validity_calibration import (
    ...
    build_sr_history,
    build_dense_history_features,
    true_bucket_from_history,
...
def evaluate_model_filtered(
    ...
    for time_idx, snap_size in enumerate(snapshot_sizes):
        ...
        seen, dt, freq = build_dense_history_features(
        ...
        logits, _ = model(snap_scores, rel_ids, seen, dt, freq)
        ...
        for i in range(logits.size(0)):
            ...
            bucket = true_bucket_from_history(s, r, o, t, sr_hist)
            ...
            if pred_o != o:
                ...
                pred_bucket = true_bucket_from_history(s, r, pred_o, t, sr_hist)
    ...
    overall_out = {
        ...
        "MRR": overall["MRR"] / max(overall["count"], 1),
        "Hits@1": overall["Hits@1"] / max(overall["count"], 1),
        "Hits@3": overall["Hits@3"] / max(overall["count"], 1),
        "Hits@10": overall["Hits@10"] / max(overall["count"], 1),
    }
    ...
    for r, v in relation_error.items():
        rate = 0.0 if v["errors"] == 0 else v["stale_repeat_errors"] / v["errors"]
        rel_rows.append({
            "relation": int(r),
            "errors": int(v["errors"]),
            "stale_repeat_error_rate": float(rate),
        })
...
@torch.no_grad()
def stale_top1_interference(model, scores_np, triples_np, sr_hist, device, batch_size=32):
    model.eval()
    ...
    for start in range(0, n, batch_size):
        ...
        logits, _ = model(batch_scores, rel_ids, seen, dt, freq)
        top1 = logits.argmax(dim=1)

        for i in range(logits.size(0)):
            s, r, o, t = map(int, batch_triples[i])
            true_bucket = true_bucket_from_history(s, r, o, t, sr_hist)

            if true_bucket in {"novel", "near"}:
                total += 1
                pred_o = int(top1[i].item())
                pred_bucket = true_bucket_from_history(s, r, pred_o, t, sr_hist)
                if pred_bucket == "stale":
    ...
    return {
        ...
        "stale_top1_rate": 0.0 if total == 0 else stale_top1 / total,
    }
...
def train_calibrator(
    ...
    for epoch in range(epochs):
        ...
        print(f"epoch {epoch + 1}: calibration_loss = {total_loss / max(total_count, 1):.6f}")
...
def main():
    ...
    hist_train = build_sr_history(train_aug)
    hist_train_valid = build_sr_history(train_valid_aug)
