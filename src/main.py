...
def run_experiment(args, history_len=None, n_layers=None, dropout=None, n_bases=None, angle=None, history_rate=None):
    ...
    if args.eval_mode == "dump_valid":
        if not os.path.exists(model_state_file):
            raise FileNotFoundError("Best checkpoint not found for validation dump: {}".format(model_state_file))
        ...
        if not os.path.exists(model_state_file):
            raise FileNotFoundError("Best checkpoint not found for test dump: {}".format(model_state_file))
        if not args.dump_full_scores:
