...
class RelationHistoryValidityCalibrator(nn.Module):
    def __init__(self, num_relations: int, mode: str = "full"):
        ...
        self.gamma_exact = nn.Parameter(torch.tensor(0.05))
        self.gamma_near  = nn.Parameter(torch.tensor(0.03))

        for emb in [
            self.rel_lambda_sr, self.rel_lambda_so, self.rel_lambda_ro
        ]:
            nn.init.constant_(emb.weight, 0.05)

        for emb in [
            self.rel_w_rec_sr, self.rel_w_rec_so, self.rel_w_rec_ro
        ]:
            nn.init.constant_(emb.weight, 1.0)

        for emb in [
            self.rel_w_freq_sr, self.rel_w_freq_so, self.rel_w_freq_ro
        ]:
            nn.init.constant_(emb.weight, 0.25)

        for emb in [
            self.rel_bias_sr, self.rel_bias_so, self.rel_bias_ro
        ]:
    ...
    def _branch(self, rel_ids, seen, dt, freq, emb_lambda, emb_wrec, emb_wfreq, emb_bias):
        ...
        freq_feat = torch.log1p(torch.clamp(freq, min=0.0)) * seen
